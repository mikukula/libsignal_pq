...
async fn async_main() -> Result<(), SignalProtocolError> {
    ...
    .with_identity_swoosh_key(*bob_identity.public_swoosh_key().unwrap());
...
mod tests {
    ...
    #[tokio::test]
    async fn benchmark_swoosh_performance() -> Result<(), SignalProtocolError> {
        ...
        for (size_name, message_size) in &message_sizes {
            ...
            for iteration in 0..num_iterations {
                ...
                bundle_memory.total_storage = bundle_memory.bundle_size + bundle_memory.key_size + 
                    bundle_memory.signature_size + bundle_memory.swoosh_key_size;
        ...
        fn calculate_memory_stats(memories: &[MemoryStats]) -> (f64, f64, f64, f64, f64, f64, f64, f64) {
            ...
            (avg_key, avg_ciphertext, avg_signature, avg_bundle, avg_session, avg_swoosh, avg_total, max_total)
        }
        ...
        for operation in &operations {
            ...
            println!("{:<12} {:<12} {:<12} {:<12} {:<12} {:<12} {:<12} {:<12} {:<12}", 
                     "Size", "Avg Key", "Avg Cipher", "Avg Sig", "Avg Bundle", "Avg Session", "Avg Swoosh", "Avg Total", "Max Total");
            println!("{}", "-".repeat(120));
            
            for (size_name, _) in &message_sizes {
                let key = format!("{}_{}", size_name, operation);
                if let Some(memories) = memory_results.get(&key) {
                    let (avg_key, avg_ciphertext, avg_signature, avg_bundle, avg_session, avg_swoosh, avg_total, max_total) = 
        ...
        for (size_name, message_size) in &message_sizes {
            ...
            if let Some(memories) = memory_results.get(&format!("{}_first_message_encrypt", size_name)) {
                let (_, avg_ciphertext, _, _, _, _, _, _) = calculate_memory_stats(memories);
        ...
        if let Some(memories) = memory_results.get("Tiny_prekey_bundle_processing") {
            let (avg_key, _, avg_signature, _, _, avg_swoosh, _, _) = calculate_memory_stats(memories);
            ...
            println!("• Total Post-Quantum Key Material: {}", format_bytes(avg_swoosh));
            println!("• Total Combined Key Material: {}", format_bytes(avg_key + avg_signature + avg_swoosh));
            let pq_overhead = if avg_key + avg_signature > 0.0 {
                (avg_swoosh / (avg_key + avg_signature)) * 100.0
            } else {
                0.0
            };
        ...
        for operation in &["prekey_message_decrypt", "signal_message_decrypt"] {
            if let Some(memories) = memory_results.get(&format!("Tiny_{}", operation)) {
                let (_, _, _, _, avg_session, _, _, _) = calculate_memory_stats(memories);
        ...
        for (size_name, message_size) in &message_sizes {
            ...
            if let Some(memories) = memory_results.get(&format!("{}_first_message_encrypt", size_name)) {
                let (_, _, _, _, _, _, avg_total, _) = calculate_memory_stats(memories);
                if avg_total > 0.0 {
        ...
        for (size_name, message_size) in message_sizes.iter().take(1) { // Just show for one size to avoid repetition
            if let Some(memories) = memory_results.get(&format!("{}_prekey_bundle_processing", size_name)) {
                ...
                println!("• Post-quantum keys: {} Swoosh = {} total", 
                         format_bytes(avg_swoosh), format_bytes(avg_swoosh));
                let pq_increase = if avg_key + avg_signature > 0.0 {
                    (avg_swoosh / (avg_key + avg_signature)) - 1.0
                } else {
                    0.0
                };
        ...
        println!("\nTotal memory measurements: {}", memory_results.values().map(|v| v.len()).sum::<usize>());
        println!("Cryptography type: Hybrid (X25519 + Swoosh) with post-quantum security");
